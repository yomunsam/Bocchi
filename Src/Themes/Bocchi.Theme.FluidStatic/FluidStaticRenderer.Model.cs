...
public sealed partial class FluidStaticRenderer
{
    ...
    /// <summary>浏览器端语言切换需要同步的公共文案 key；Theme 私有 key 从当前 manifest 动态收集。</summary>
    private static readonly string[] CommonClientI18nKeys =
    [
        "menu.home",
        "menu.posts",
        "menu.works",
        ...
        "common.appearance",
        "common.openMenu",
        "common.appearanceAuto",
        "common.appearanceLight",
        "common.appearanceDark",
    ...
    private static Dictionary<string, object?> CreateListingModel(
        ...
        string emptyText)
    {
        var model = CreatePageModel(site, text, theme, title, currentPath);
        ...
        model["emptyTextKey"] = ThemeTextKey(theme, "emptyList");
        return model;
    }
        ...
        JsonElement? contentItem = null)
    {
        ...
        return new Dictionary<string, object?>(StringComparer.Ordinal)
        {
            ...
            ["text"] = CreateTextModel(text, theme),
    ...
    /// <summary>创建公开给模板的 Theme 身份与完整有效配置模型；身份只信任已解析的 manifest。</summary>
    private static IReadOnlyDictionary<string, object?> CreateThemeModel(ThemeManifest manifest, JsonElement themeContext)
    {
        var config = TryGetPath(themeContext, ["theme", "config"]);
        return new Dictionary<string, object?>(StringComparer.Ordinal)
        {
            ["id"] = manifest.Id,
            ["name"] = manifest.Name,
            ...
            ["config"] = config is null
                ...
                : ToTemplateValue(config.Value),
        };
    }

    /// <summary>把任意 JSON 配置递归转换为 Fluid 可直接访问的字典、数组和标量。</summary>
    private static object? ToTemplateValue(JsonElement value)
    {
        return value.ValueKind switch
        {
            JsonValueKind.Object => value.EnumerateObject().ToDictionary(
                property => property.Name,
                property => ToTemplateValue(property.Value),
                StringComparer.Ordinal),
            JsonValueKind.Array => value.EnumerateArray().Select(ToTemplateValue).ToArray(),
        ...
        IReadOnlyDictionary<string, string> configTextFormats)
    {
        ...
        {
            ["heroTitle"] = title,
            ["heroTitleHtml"] = FluidStaticInlineTextRenderer.Render(title, titleFormat),
            ["heroTitleKey"] = HomeHeroTitleClientKey,
            ["heroTitleFormat"] = ToClientTextFormat(titleFormat),
            ["heroSubtitle"] = subtitle,
            ["heroSubtitleHtml"] = FluidStaticInlineTextRenderer.Render(subtitle, subtitleFormat),
    ...
    private static string GetTextFormat(IReadOnlyDictionary<string, string> formats, string key)
        => formats.TryGetValue(key, out var format) ? format : FluidStaticInlineTextRenderer.PlainFormat;

    /// <summary>转换为前端 data attribute；plain 不输出属性，保持普通 i18n 路径。</summary>
    private static string ToClientTextFormat(string format)
        => FluidStaticInlineTextRenderer.IsInlineColorFormat(format) ? FluidStaticInlineTextRenderer.InlineColorFormat : string.Empty;
    ...
    private static string ResolveLocalizedText(Dictionary<string, string> values, FluidStaticTextResolver text)
    {
    ...
    private static string[] ResolveLocalizedList(Dictionary<string, string[]> values, FluidStaticTextResolver text)
    {
        foreach (var language in CreateConfigLanguageFallbacks(values.Keys, text))
        {
            if (values.TryGetValue(language, out var value))
    ...
    private static IEnumerable<string> CreateConfigLanguageFallbacks(IEnumerable<string> availableLanguages, FluidStaticTextResolver text)
    {
    ...
    private static Dictionary<string, object?> CreateTextModel(
        FluidStaticTextResolver text,
        IReadOnlyDictionary<string, object?> theme)
    {
        return new Dictionary<string, object?>(StringComparer.Ordinal)
        {
            ["homeSelectedWriting"] = text.Get(ThemeTextKey(theme, "homeSelectedWriting")),
            ["homeSelectedWork"] = text.Get(ThemeTextKey(theme, "homeSelectedWork")),
            ["homeRecentNotes"] = text.Get(ThemeTextKey(theme, "homeRecentNotes")),
            ["homeFriends"] = text.Get("menu.friends"),
            ["all"] = text.Get(ThemeTextKey(theme, "all")),
            ["emptyList"] = text.Get(ThemeTextKey(theme, "emptyList")),
            ["colophonBuiltWith"] = text.Get(ThemeTextKey(theme, "colophonBuiltWith")),
            ["linkLabel"] = text.Get(ThemeTextKey(theme, "linkLabel")),
            ["pageLabel"] = text.Get(ThemeTextKey(theme, "pageLabel")),
            ["articleBackPrefix"] = text.Get(ThemeTextKey(theme, "articleBackPrefix")),
            ["toggleAppearance"] = text.Get(ThemeTextKey(theme, "toggleAppearance")),
            ["openMenu"] = text.Get("common.openMenu"),
            ["languageLabel"] = text.Get("common.language"),
            ["languageSwitchLabel"] = text.Get(ThemeTextKey(theme, "languageSwitchLabel")),
            ["currentLanguageLabel"] = text.Get(ThemeTextKey(theme, "currentLanguageLabel")),
            ["appearanceLabel"] = text.Get("common.appearance"),
            ["appearanceAuto"] = text.Get("common.appearanceAuto"),
            ["appearanceLight"] = text.Get("common.appearanceLight"),
            ["appearanceDark"] = text.Get("common.appearanceDark"),
        ...
        IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>>? extraText = null)
    {
        ...
        var clientKeys = CommonClientI18nKeys.Concat(text.ThemeKeys);
        if (navigation is not null)
        {
            clientKeys = clientKeys.Concat(EnumerateNavigationI18nKeys(navigation));
        }
    ...
    /// <summary>按当前 Theme id 创建私有 i18n key，确保公共 renderer 不绑定具体 Theme namespace。</summary>
    private static string ThemeTextKey(IReadOnlyDictionary<string, object?> theme, string suffix)
        => $"theme.{theme["id"]}.{suffix}";
