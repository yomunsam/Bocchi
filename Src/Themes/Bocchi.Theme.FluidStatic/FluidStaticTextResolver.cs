...
internal sealed record FluidStaticLanguage
{
...
internal sealed class FluidStaticTextResolver
{
    ...
    public FluidStaticLanguage[] EnabledLanguages { get; }

    /// <summary>当前 Theme manifest 声明的全部私有 i18n key。</summary>
    public IEnumerable<string> ThemeKeys => _themeDefaults.Keys;

    /// <summary>构造前台文案解析器。</summary>
    private FluidStaticTextResolver(
        string currentLanguage,
        string primaryLanguage,
        FluidStaticLanguage[] enabledLanguages,
    ...
    public static FluidStaticTextResolver From(JsonElement context)
    {
        ...
        return new FluidStaticTextResolver(
            normalizedPrimary,
            normalizedPrimary,
    ...
    public FluidStaticTextResolver WithCurrentLanguage(string? language)
    {
        ...
        return string.Equals(normalized, CurrentLanguage, StringComparison.OrdinalIgnoreCase)
            ? this
            : new FluidStaticTextResolver(normalized, PrimaryLanguage, EnabledLanguages, _overrides, _themeDefaults, _themeDefaultLanguage);
    }
    ...
    private static Dictionary<string, Dictionary<string, string>> CreateCommonTextDefaults()
    {
        return new Dictionary<string, Dictionary<string, string>>(StringComparer.Ordinal)
        {
            ...
            ["common.language"] = CreateLanguageValues("Language", "语言", "語言", "言語"),
            ["common.appearance"] = CreateLanguageValues("Appearance", "外观", "外觀", "外観"),
            ["common.openMenu"] = CreateLanguageValues("Open menu", "打开菜单", "開啟選單", "メニューを開く"),
            ["common.appearanceAuto"] = CreateLanguageValues("Auto", "自动", "自動", "自動"),
            ["common.appearanceLight"] = CreateLanguageValues("Light", "浅色", "淺色", "ライト"),
            ["common.appearanceDark"] = CreateLanguageValues("Dark", "深色", "深色", "ダーク"),
    ...
    private static FluidStaticLanguage[] ReadEnabledLanguages(JsonElement context, string primaryLanguage)
    {
        var result = new List<FluidStaticLanguage>();
        var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        ...
        if (languages is { ValueKind: JsonValueKind.Array })
        {
            foreach (var language in languages.Value.EnumerateArray())
            {
                ...
                result.Add(new FluidStaticLanguage
                {
                    Code = code.Trim(),
        ...
        {
            result.Insert(0, new FluidStaticLanguage
            {
