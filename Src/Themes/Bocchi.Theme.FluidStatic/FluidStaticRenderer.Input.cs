...
public sealed partial class FluidStaticRenderer
{
    ...
    private static async Task<JsonElement> ReadDataAsync(string inputDirectory, string fileName, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                throw new FluidStaticException($"Theme 输入 '{fileName}' contractVersion='{version}'，当前只支持 '{ThemeContractVersion.Current}'。");
            }
        ...
        {
            throw new FluidStaticException($"Theme 输入 '{fileName}' 不是合法 JSON。", ex);
        }
    }
    ...
    private static async Task<JsonElement[]> ReadArrayAsync(string inputDirectory, string fileName, CancellationToken cancellationToken)
    {
        ...
        {
            throw new FluidStaticException($"Theme 输入 '{fileName}' 的 data 必须是 array。");
        }
    ...
    {
        var schema = await TryReadConfigSchemaAsync(themeRoot, cancellationToken).ConfigureAwait(false);
        if (schema is null)
        {
        ...
        foreach (var group in groups.EnumerateArray().Where(group => group.ValueKind == JsonValueKind.Object))
        {
            ...
            foreach (var field in fields.EnumerateArray().Where(field => field.ValueKind == JsonValueKind.Object))
            {
                ...
                var format = FluidStaticInlineTextRenderer.NormalizeFormat(GetString(field, "textFormat"));
                if (!string.IsNullOrWhiteSpace(key) &&
                    IsTextFormatEligibleField(type) &&
                    FluidStaticInlineTextRenderer.IsInlineColorFormat(format))
                {
    ...
    /// <summary>读取当前 Theme 自己的可选 schema；缺失时按无格式声明处理。</summary>
