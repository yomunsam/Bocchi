...
public sealed class FluidStaticRendererTests
{
    ...
    /// <summary>缺少标准模板时必须报告当前 Theme 自身不完整，不能从内置 Theme 资源静默补齐。</summary>
    [Fact]
    public async Task RenderAsync_WhenRequiredTemplateIsMissing_FailsWithoutBuiltInFallback()
    {
        var act = () => RenderIndexAsync(
            CreateThemeContext(),
            configureTheme: themeRoot => File.Delete(Path.Combine(themeRoot, "templates", "pages", "posts.liquid")));

        await act.Should().ThrowAsync<FluidStaticException>()
            .WithMessage("*Fluid Static Theme 缺少必需模板 'templates/pages/posts.liquid'*");
    }

    /// <summary>验证公共 Fluid Static model 暴露当前 Theme 身份与任意 config 字段，不依赖默认 Theme 的固定 schema。</summary>
    [Fact]
    public async Task RenderAsync_ExposesActiveThemeIdentityAndArbitraryConfig()
    {
        ...
        var manifest = CreateManifest("cozy", "Cozy", "2.3.4");

        var html = await RenderIndexAsync(
            CreateThemeContext(themeId: manifest.Id, themeConfig: config),
            manifest: manifest,
            configureTheme: themeRoot => File.WriteAllText(
                Path.Combine(themeRoot, "templates", "pages", "index.liquid"),
                "{{ theme.id }}|{{ theme.name }}|{{ theme.version }}|{{ theme.config.custom.cardLabel }}"));

        html.Should().Contain("cozy|Cozy|2.3.4|Cozy card");
    }

    /// <summary>验证 Theme 模板可以通过公共 t filter 解析自己的私有 i18n key。</summary>
    [Fact]
    public async Task RenderAsync_ResolvesThemeTextThroughTFilter()
    {
        var manifest = CreateManifest("cozy", "Cozy", "2.3.4");
        var context = CreateThemeContext(
            ...
            themeI18nKeys:
            [
                CreateThemeI18nKey("theme.cozy.cardLabel", ("zh-CN", "舒适卡片")),
            ]);
        ...
        var html = await RenderIndexAsync(
            ...
            configureTheme: themeRoot => File.WriteAllText(
                Path.Combine(themeRoot, "templates", "pages", "index.liquid"),
                "{{ \"theme.cozy.cardLabel\" | t }}"));

        System.Net.WebUtility.HtmlDecode(html).Should().Contain("舒适卡片");
    }

    /// <summary>验证客户端语言包只收集当前 manifest 声明的 Theme 私有 key，不注入具体内置 Theme key。</summary>
    [Fact]
    public async Task RenderAsync_IncludesManifestThemeKeysInClientTextJson()
    {
        var manifest = CreateManifest("cozy", "Cozy", "2.3.4");
        var context = CreateThemeContext(
            themeId: manifest.Id,
        ...
        var html = await RenderIndexAsync(context, manifest: manifest);

        var clientText = ReadClientI18nText(html);
        clientText.GetProperty("theme.cozy.cardLabel")
            .GetProperty("zh-CN")
            .GetString()
            .Should().Be("舒适卡片");
        clientText.EnumerateObject()
            .Select(property => property.Name)
            .Should().NotContain(key => key.StartsWith("theme.bocchi-mono", StringComparison.Ordinal));
    }

    /// <summary>验证 Fluid Static build 总是写出共享 runtime，并为根页面和嵌套页面生成正确相对 URL。</summary>
    [Fact]
    public async Task RenderAsync_WritesSharedRuntimeAndRelativizesScriptUrl()
    {
        string? runtimeSource = null;
        var indexHtml = await RenderIndexAsync(
            CreateThemeContext(),
            inspectOutput: outputDirectory => runtimeSource = File.ReadAllText(
                Path.Combine(outputDirectory, "_bocchi", "fluid-static-v1.js")));
        var articleHtml = await RenderSinglePostArticleAsync(
            showUpdatedAt: true,
            updatedAt: "2026-05-28T00:30:00Z");

        runtimeSource.Should().Contain("bocchi:languagechange");
        indexHtml.Should().Contain("src=\"_bocchi/fluid-static-v1.js\"");
        articleHtml.Should().Contain("src=\"../../../_bocchi/fluid-static-v1.js\"");
    }
    ...
    public async Task RenderAsync_ResolvesThemeTextOverridesDefaultsAndFallbacks()
    {
        var context = CreateThemeContext(
            localizationText: new Dictionary<string, object?>(StringComparer.Ordinal)
            {
                ...
                ["theme.bocchi-mono.colophonBuiltWith"] = CreateLanguageText(("zh-CN", "主题私有覆盖")),
            },
            themeI18nKeys:
            [
                CreateThemeI18nKey("theme.bocchi-mono.colophonBuiltWith", ("zh-CN", "Manifest 页脚"), ("en-US", "Manifest footer")),
                CreateThemeI18nKey("theme.bocchi-mono.homeSelectedWriting", ("zh-CN", "Manifest 精选写作"), ("en-US", "Manifest selected writing")),
                CreateThemeI18nKey("theme.bocchi-mono.homeSelectedWork", ("en-US", "Manifest selected work")),
            ]);
        ...
        visibleHtml.Should().Contain("""data-bocchi-i18n="theme.bocchi-mono.colophonBuiltWith">主题私有覆盖</span>""");
        visibleHtml.Should().Contain("""data-bocchi-i18n="theme.bocchi-mono.homeSelectedWriting">Manifest 精选写作</span>""");
        visibleHtml.Should().Contain("""data-bocchi-i18n="theme.bocchi-mono.homeSelectedWork">Manifest selected work</span>""");
        visibleHtml.Should().Contain("""data-bocchi-i18n="theme.bocchi-mono.emptyList">theme.bocchi-mono.emptyList</div>""");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "bocchi-fluid-static-" + Guid.NewGuid().ToString("N"));
        try
        {
            ...
            CopyDefaultThemeFiles(themeRoot);
    ...
    private static JsonElement ReadClientI18nValues(string html, string key)
        => ReadClientI18nText(html).GetProperty(key).Clone();

    /// <summary>读取页面内嵌客户端 i18n JSON 的完整 text 对象。</summary>
    private static JsonElement ReadClientI18nText(string html)
    {
    ...
    private static Dictionary<string, object?> CreateThemeContext(
        ...
        string themeId = "bocchi-mono",
        IReadOnlyDictionary<string, object?>? themeConfig = null)
        => new(StringComparer.Ordinal)
        {
            ...
            {
                ["id"] = themeId,
                ["config"] = themeConfig ?? new Dictionary<string, object?>(StringComparer.Ordinal)
                {
    ...
    private static async Task<string> RenderIndexAsync(
        Dictionary<string, object?> themeContext,
        object? navigation = null,
        Action<string>? configureTheme = null,
        ...
        Action<string>? inspectOutput = null)
    {
        ...
        try
        {
            ...
            Directory.CreateDirectory(inputDirectory);
            CopyDefaultThemeFiles(themeRoot);
            configureTheme?.Invoke(themeRoot);
            ...
            inspectOutput?.Invoke(outputDirectory);
    ...
    /// <summary>把仓库中的具体默认 Theme 模板和 schema 显式复制到测试 Theme Root，避免测试依赖 renderer 的跨 Theme fallback。</summary>
    private static void CopyDefaultThemeFiles(string themeRoot)
    {
        var repositoryRoot = FindRepositoryRoot();
        var sourceRoot = Path.Combine(repositoryRoot, "Themes", "bocchi-mono");
        CopyDirectory(Path.Combine(sourceRoot, "templates"), Path.Combine(themeRoot, "templates"));
        File.Copy(Path.Combine(sourceRoot, "config-schema.json"), Path.Combine(themeRoot, "config-schema.json"), overwrite: true);
    }

    /// <summary>从测试程序集目录向上定位 solution 根，避免依赖 dotnet test 的当前工作目录。</summary>
    private static string FindRepositoryRoot()
    {
        for (var directory = new DirectoryInfo(AppContext.BaseDirectory); directory is not null; directory = directory.Parent)
        {
            if (File.Exists(Path.Combine(directory.FullName, "Bocchi.slnx")))
            {
                return directory.FullName;
            }
        }

        throw new InvalidOperationException("无法定位 Bocchi solution 根目录。");
    }

    /// <summary>递归复制测试需要的 Theme 源目录，并保留相对结构。</summary>
