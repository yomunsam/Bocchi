...
- **workspace**：用户的纯创作资产 —— Blog、独立页面、作品集、短文、友链、站点设置。它必须满足：
  ...
  - 仅包含原始 Markdown 与原始媒体；**禁止出现任何构建产物 / 派生媒体**（如系统生成的 webp variants、缩略图、HTML、搜索索引）。
...
- workspace 内禁止出现系统派生产物（如缩略图、预览图、响应式 variants）；扫描器会发出 warning。用户上传的原始媒体保持原始格式，不能因为扩展名是 webp / avif 就视为派生产物。
...
workspace 是原始媒体事实源，构建产物不写回 workspace。Generator 可以在 `<data>/cache/derivatives/` 中生成 webp、缩略图、预览图、响应式宽度档位等派生产物，并在输出阶段登记为 `/media/...` artifact。

媒体派生由 Generator / 构建管线负责，Theme 不直接执行图片转码。Theme 通过正文 HTML、媒体 variants 和 Theme config 决定展示方式，例如 lightbox、lazy load、占位动画、默认 Figure 大小和对齐。

普通图片、Figure、封面和 Note media 都应共享同一套媒体输出策略。animated gif 可以优先尝试 animated webp；无法可靠转换或转换收益差时保留原始 gif 作为输出 fallback。
...
- `builtIn`：随 Bocchi 分发并在需要时物化到 `<data>/themes/bocchi-mono/` 的内置默认 Theme。
...
4. `bocchi-mono` 在需要时由 `Bocchi.Themes.BuiltIn.Bundle` 补齐到 DataRoot。
...
```json
{
  ...
  "pageTemplates": [
    {
      "name": "normal",
      "displayName": "i18n://theme@theme.bocchi-mono.pageTemplate.normal"
    },
    {
      "name": "about",
      "displayName": "i18n://theme@theme.bocchi-mono.pageTemplate.about"
    }
  ],
  "specialPages": [
    {
...
`bocchi-mono` 是内置默认 Theme，不能通过 zip 覆盖。第三方 Theme 应使用独立 id 和完整、自包含的 Theme Root。
...
- `fluid-static`：调用 Bocchi 随包分发的 Fluid 静态站点 renderer；Theme 只需提供静态文件，不需要 .NET 或 Node.js 项目依赖。
...
默认 Theme 使用 Fluid/Liquid 风格模板 renderer，canonical source 位于仓库 `Themes/bocchi-mono/`。发布时由 `Bocchi.Themes.BuiltIn.Bundle` 作为 embedded resources 随 Home Server 分发，运行时物化到 `<data>/themes/bocchi-mono/`。Bundle 只负责携带和物化资源，不实现 Fluid Static renderer。
...
- `Bocchi.Theme.FluidStatic` 只承载公开 Fluid Static profile；`Bocchi.Themes.BuiltIn.Bundle` 只携带内置 Theme 资源，二者互不引用。
- Bocchi Mono 是完整的具体 Theme；独立 Cozy repo 是第三方 Fluid Static 参考实现，不继承也不读取 Bocchi Mono。
